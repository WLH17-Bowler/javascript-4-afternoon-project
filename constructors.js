...
function Employee(name, email, hireDate) {
  ...
  */
  ...
  Movie.prototype.changeRating = function(number) {
    this.rating = (number + this.rating) / 2
    return this.rating
  }

  ////////// PROBLEM 5 //////////
  
  // Write a constructor function called User. This function should take in 4 parameters called name, age, email, and savedPosts in that order. 
  ...
  // Name this method addSavedPost. It should take in three parameters: id (a number), title (a string) and rating (a number). 
  ...
  User.prototype.addSavedPost = function(id, title, rating) {
    return this.savedPosts.push({id: id, title: title, rating: rating})
  }
  
  ////////// PROBLEM 6 //////////
  
  // You will be using the constructor function you just created in problem 5.
  // Write a prototype method for the User constructor function named removeSavedPost that will take in one number parameter representing the post id. 
  // Use this id to find and remove the matching object in the savedPosts array.
  
  User.prototype.removeSavedPost = function(id) {
    this.savedPosts = this.savedPosts.filter(element => element.id !== id)
  }
  console.log(User)
  ...
  // Write a prototype method for the User constructor function named changePostRating that will take in two number parameters. 
  // The first will be an id (a number) and the second will be the new rating (a number). 
  ...
  User.prototype.changePostRating = function(id, rating) {
    let currentPost = this.savedPosts.filter(element => element.id === id)[0]
    currentPost.rating = rating
    return currentPost
  }


  // let bobsTotal = purchases.filter((element) => element.owner === 'Bob').map(element => element.price).reduce((accu, element) => accu + element)